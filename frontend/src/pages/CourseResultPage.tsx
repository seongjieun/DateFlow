...
export default function CourseResultPage() {
    ...
    useEffect(() => {
        // 1. location.state 우선 (온보딩 → /courses/generate → navigate 흐름)
        const cd = (state as any)?.courseData;
        if (cd) {
            const tagsM: string[] = cd.tags_m || [];
            const tagsF: string[] = cd.tags_f || [];
            const budget: number  = cd.budget_max || 100000;
            const weather         = (cd.weather || "cloudy") as WeatherType;
            setCourseData(prev => ({
                ...prev,
                title:   `${cd.region} 데이트 코스`,
                personA: tagsM.length > 0 ? tagsM : prev.personA,
                personB: tagsF.length > 0 ? tagsF : prev.personB,
                common:  tagsM.filter((t: string) => tagsF.includes(t)),
                budget,
                weather: { precipitationRate: 0, weatherType: weather, description: weather },
            }));
            const backendPlaces = cd.courses?.[0]?.places || [];
            const adapted = adaptPlaces(backendPlaces, budget);
            if (adapted.length > 0) setPlaces(adapted);
            return;
        }

        // 2. URL 파라미터 fallback (레거시 호환)
        ...
        setIsLoading(true);
        fetch(`${API_BASE}/courses/generate`, {
            method:  "POST",
