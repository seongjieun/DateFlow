...
function adaptPlaces(
  ...
  return backendPlaces.map((p, i) => {
    ...
    return {
      ...
      lon: p.longitude ? parseFloat(p.longitude) : 126.978,
      estimatedCost: cost,
...
function initCourseData(): CourseResult {
  ...
  return {
    ...
    common:
      tagsA.length > 0 || tagsB.length > 0 ? common : mockCourseData.common,
    budget,
    weather: {
...
export default function CourseResultPage() {
  ...
  useEffect(() => {
    ...
    const budget =
      bMaxA > 0 && bMaxB > 0 ? bMaxA + bMaxB : mockCourseData.budget;
    ...
    fetch(`${API_BASE}/course/generate`, {
      ...
      .then((data) => {
        const backendPlacesA = data.courses?.[0]?.places || [];
        const backendPlacesB = data.courses?.[1]?.places || [];
        const adaptedA = adaptPlaces(backendPlacesA, budget);
        const adaptedB = adaptPlaces(backendPlacesB, budget);
        if (adaptedA.length > 0) setPlacesA(adaptedA);
        if (adaptedB.length > 0) setPlacesB(adaptedB);
      })
      .catch(() => {})
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          {isLoading && (
            <p
              style={{ fontSize: "12px", color: "#B8A9D9", margin: "2px 0 0" }}
            >
              코스 불러오는 중...
            </p>
          )}
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "24px" }}>
        {(["A", "B"] as CourseTab[]).map((tab) => {
          const label =
            tab === "A" ? "A안 · 차분한 감성 코스" : "B안 · 액티브 핫플 코스";
      ...
      {(() => {
        ...
        const fmtW = (v: number) =>
          v >= 999999 ? "제한없음" : `${Math.round(v / 10000)}만원`;
          ...
          >
            ...
            >
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <span
                  style={{ fontSize: "11px", color: "#bbb", marginLeft: "6px" }}
                >
                  {rangeLabel}
                </span>
              </div>
          ...
          {closedWarnings.length > 0 && (
            ...
            >
              {closedWarnings.map((p) => (
                ...
                >
                  ⚠ <strong>{p.name}</strong> — {p.closedDays} 휴무예요. 예약 전
                  확인하세요.
                </div>
              ))}
            </div>
